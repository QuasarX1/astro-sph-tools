...
#        return self.__data_read_reorder[particle_type](self.__file_object.read_dataset(particle_type.value, "ParticleIDs"))
    ...
    def get_smoothing_lengths(self, particle_type: ParticleType, use_proper_units: bool = False) -> unyt_array:
        return self._convert_to_cgs_length(self.__read_dataset(particle_type, "SmoothingLength", expected_dtype = float), propper = use_proper_units).to("Mpc")
    ...
    def get_total_black_hole_subgrid_mass(self) -> unyt_array:
        return self.get_black_hole_subgrid_masses().sum()

    def get_total_black_hole_dynamical_mass(self) -> unyt_array:
    ...
    def _get_number_densities(self, particle_type: ParticleType, element: str, use_proper_units: bool, default_abundance: float|None = None) -> unyt_array:
        if self.snipshot and default_abundance is None:
            raise KeyError("Unable to read abundance data - snipshots do not contain this information. A \"default_abundance\" value must be specified.")
    ...
    def _get_elemental_abundance(self, particle_type: ParticleType, element: str) -> unyt_array:
        if self.snipshot:
        ...
        return self.__read_dataset(particle_type, f"ElementAbundance/{element}", expected_dtype = float)
